...
pub async fn loop_forever(graph: impl NetworkGraphDB, args: ConnectorArgs) {
    if let Err(error) = try_loop_forever(graph, args).await {
        error!("failed to run connect job: {error}")
    }
}

async fn try_loop_forever(graph: impl NetworkGraphDB, args: ConnectorArgs) -> Result<()> {
    let connector = NetworkConnector::try_new(&args)?;
    ::kubegraph_api::connector::NetworkConnector::loop_forever(connector, graph).await;
    Ok(())
}
...
impl NetworkConnector {
    fn try_new(args: &ConnectorArgs) -> Result<Self> {
