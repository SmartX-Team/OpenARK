...
#[cfg(feature = "local")]
type DefaultNetworkGraphDB = ::kubegraph_db_local::NetworkGraphDB;
...
struct Args {
    ...
    #[command(flatten)]
    graph: ::kubegraph_db_local::NetworkGraphDBArgs,
}
...
async fn main() {
    ...
    let graph = match DefaultNetworkGraphDB::try_new(&graph).await {
        Ok(graph) => graph,
        Err(error) => {
