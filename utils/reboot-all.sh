...
IPMITOOL_IMAGE_DEFAULT="quay.io/ulagbulag/openark-ipmitool:latest"
...
for address in $(kubectl get box -o jsonpath='{.items[*].spec.power.address}'); do
    ...
    then
        ...
        "${CONTAINER_RUNTIME}" run --rm --net "host" "${IPMITOOL_IMAGE}" \
            -H "${address}" -U "kiss" -P "kiss.OpenARK" chassis bootparam set bootflag force_pxe >/dev/null
        "${CONTAINER_RUNTIME}" run --rm --net "host" "${IPMITOOL_IMAGE}" \
            -H "${address}" -U "kiss" -P "kiss.OpenARK" chassis bootdev pxe options=persistent,efiboot >/dev/null

        # Reboot now anyway
        "${CONTAINER_RUNTIME}" run --rm --net "host" "${IPMITOOL_IMAGE}" \
            -H "${address}" -U "kiss" -P "kiss.OpenARK" power cycle >/dev/null
        echo "OK"
    else
        echo "Skipped"
    fi
