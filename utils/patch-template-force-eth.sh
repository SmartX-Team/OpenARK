...
if [ "x${ethernet_name}" != 'x' ] && [ "x${wlan_name}" != 'x' ]; then
    ...
    if [ "x${ethernet_mac}" != 'x' ]; then
        cd /etc/udev/rules.d/
        udev_dst="./70-kiss-net-setup-link-master.rules"

        udev_master="./70-kiss-net-setup-link-${wlan_name}.rules"
        ...
        enable_master="./10-kiss-enable-${wlan_name}.nmconnection"
        ...
        if [ -f "${disable_master}" ]; then
            if ip a show dev "${ethernet_name}" | grep -q 'state DOWN'; then
        ...
        if [ -f "${disable_src}" ]; then
            ...
            sudo sed -i "s/\(disable-\)master/\1${wlan_name}/g" "${disable_dst}"
            sudo sed -i "s/\=ethernet/\=wifi/g" "${disable_dst}"
            sudo sed -i "s/${ethernet_name}/${wlan_name}/g" "${disable_dst}"
            sudo sed -i "s/\(mac-address=\)[0-9a-f:]\+/\1${wlan_mac}/g" "${disable_dst}"
            sudo sed -i "s/\(qdisc\.root=\)[_a-z]*/\1noqueue/g" "${disable_dst}"
            ...
            sudo sed -i "s/\(mac-address=\)[0-9a-f:]\+/\1${ethernet_mac}/g" "${enable_dst}"
            sudo sed -i "s/\(qdisc\.root=\)[_a-z]*/\1mq/g" "${enable_dst}"

            sudo nmcli connection reload &&
                sudo nmcli connection up '10-kiss-enable-master' &&
                sudo nmcli connection up "20-kiss-disable-${wlan_name}"
            sudo reboot
        fi
    fi
fi
