...
impl IntoFlattened for StructArray {
    fn into_flattened(self) -> Self {
        ...
        flatten_struct_array(&mut flattened_fields, "", Arc::new(self), false);
...
fn flatten_struct_array(
    ...
    match array.as_struct_opt() {
        Some(array) => {
            let (fields, arrays, nulls) = array.clone().into_parts();
            fields.into_iter().zip(arrays).for_each(|(field, array)| {
                ...
                flatten_struct_array(flattened_fields, &name, array, nullable)
            })
        }
        None => {
